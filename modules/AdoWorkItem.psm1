...
function Initialize-AdoConfig {
    ...
    if ([string]::IsNullOrWhiteSpace($script:AdoPat)) {
        throw "Initialize-AdoConfig: ADO_PAT not found in .env"
    }
    if ([string]::IsNullOrWhiteSpace($script:AdoOrg)) {
        throw "Initialize-AdoConfig: AZURE_DEVOPS_ORG not found in .env"
    }
    if ([string]::IsNullOrWhiteSpace($script:AdoProject)) {
        throw "Initialize-AdoConfig: AZURE_DEVOPS_PROJECTS not found in .env"
    }
}
...
function New-AdoWorkItem {
    ...
    try {
        $resp = Invoke-RestMethod -Uri $uri -Method Patch -ContentType 'application/json-patch+json' -Body $body -Headers @{
