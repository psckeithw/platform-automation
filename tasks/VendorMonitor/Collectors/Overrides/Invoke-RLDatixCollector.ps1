...
function Invoke-OverrideCollector {
    ...
    foreach ($g in $grouped) {
        ...
        $modules = $g.Group | ForEach-Object { $_.Product } | Where-Object { $_ -and $_ -ne ',' } | ForEach-Object { $_ -split ',' } | ForEach-Object { $_.Trim() } | Where-Object { $_ } | Sort-Object -Unique
        ...
        if ($moduleStr) {
            $summaryHtml += "<p>Affected modules: $moduleStr</p>"
        }

        if ($version -match '^\d+\.\d+\.\d+$') {
            $summaryHtml += '<p><em>Hotfix release.</em></p>'
        }
