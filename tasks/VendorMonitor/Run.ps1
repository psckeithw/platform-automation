...
try {
    ...
    # ---------- User Story creation (NEW records only, skip baseline) ----------
...
"@
                    $workItemId = New-AdoWorkItem -Title $title -DescriptionHtml $fullBody
                    Write-Log -Message "Created User Story #$workItemId for $title" -Level INFO
                    $item.workItemId = $workItemId
                }
                catch {
                    $msg = "Failed to create User Story for '$([string]$item.Title)': $($_.Exception.Message)"
                    Write-Log -Message $msg -Level ERROR
                    Write-Host "##vso[task.logissue type=error]$msg"
                }
            }
